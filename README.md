...
## 🚀 개발 과정 및 문제 해결 경험
과제 진행 중 **내지 추가 단계**에서 가장 큰 어려움을 겪었습니다. 지속적으로 에러가 발생해 처음에는 서버 불안정이라고 생각했으나, 응답 로그를 자세히 분석한 결과 `napLabelX`, `napValueX` 등 필수 파라미터가 누락되거나 잘못 입력된 것이 원인임을 발견했습니다.

이를 수정한 후 1x1px 가상 이미지를 활용하고, Exponential Backoff Retry 로직과 페이지당 12초 대기 처리를 적용하여 24페이지 내지 추가에 성공했습니다. 이후 책 확정과 주문 단계까지 전체 파이프라인을 정상적으로 완료했습니다.
